...
| 2 | `02-original-image.png` | Gambar asli yang diunggah sebelum proses segmentasi dilakukan. | Original uploaded image before the segmentation process. | Original handwritten text image displayed in the Streamlit demo. | Gunakan gambar sample bersih, jelas, dan tanpa metadata EXIF/GPS. |
| 3 | `03-threshold-preprocessing-result.png` | Hasil preprocessing berupa grayscale dan binary threshold untuk memisahkan area teks dari background. | Preprocessing result showing grayscale and binary threshold output to separate text regions from the background. | Grayscale and threshold preprocessing result for handwritten text segmentation. | Hasil harus memiliki kontras jelas antara tulisan dan background. |
...
├── 02-original-image.png
├── 03-threshold-preprocessing-result.png
├── 04-bounding-box-output.png
└── 05-segmented-words-output.png
```
